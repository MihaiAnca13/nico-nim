...
proc loadSurfaceFromPNGBytes*(data: string, callback: proc(surface: common.Surface)) =
  ...
  debug "loadSurfaceFromPNGBytes size: ", pngInfo.width, "x", pngInfo.height, " type: ", pngInfo.mode.colorType

  var surface = newSurface(pngInfo.width, pngInfo.height)
  surface.filename = "<memory>"
  surface.w = pngInfo.width
  surface.h = pngInfo.height

  if pngInfo.mode.colorType == LCT_RGBA:
    debug "loading RGBA image, converting to indexed using current palette <memory>"
    surface.channels = 4
    surface.data = cast[seq[uint8]](png.pixels)
    callback(surface.convertToIndexed())

  elif pngInfo.mode.colorType == LCT_RGB:
    debug "loading RGB image, converting to indexed using current palette <memory>"
    surface.channels = 3
    surface.data = cast[seq[uint8]](png.pixels)
    callback(surface.convertToIndexed())

  elif pngInfo.mode.colorType == LCT_PALETTE:
    debug "loading paletted image <memory>"
    surface.channels = 1
    surface.palette = @[]
    for i, c in pngInfo.mode.palette:
      surface.palette.add((c.r.uint8, c.g.uint8, c.b.uint8))
